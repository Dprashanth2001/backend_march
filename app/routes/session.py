...
def create_session(session: Session):
    ...
    if sessions_col.find_one({"session_id": session.session_id, "status": "active"}):
        raise HTTPException(400, f"Session '{session.session_id}' is already active. End it before creating a new one.")

    result = sessions_col.insert_one(session.dict())

    # ✅ Return the MongoDB _id as session_instance_id so frontend can use it
    # for response counting — this uniquely identifies each run even when
    # session_id is reused (e.g. same lab morning + afternoon)
    return {
        "status": "success",
        "message": "Session created successfully",
        "session_instance_id": str(result.inserted_id)
    }
