...
@router.post("/submit")
def submit_survey(response: SurveyResponse):
    # Always resolve session_instance_id from the currently active session
    # This is the MongoDB _id — unique per session run even if session_id is reused
    active = sessions_col.find_one({
        "session_id": response.session_id,
        "status": "active"
    })
    if not active:
        raise HTTPException(400, "No active session found. The session may have ended.")

    response.session_instance_id = str(active["_id"])

    # Check if this student already submitted for THIS specific session run
    existing = survey_responses_col.find_one({
        "student_id": response.student_id,
        "session_instance_id": response.session_instance_id
    })
    if existing:
        raise HTTPException(400, "You have already submitted a survey for this session.")
    ...
    try:
        ...
        err_str = str(e)
        # Old index student_id+session_id still in Atlas — drop it manually:
        # Atlas → Collections → survey_responses → Indexes → drop student_id_1_session_id_1
        if "student_id_1_session_id_1" in err_str:
            raise HTTPException(
                400,
                "DB index conflict: please drop the index 'student_id_1_session_id_1' "
                "from MongoDB Atlas → survey_responses → Indexes tab."
            )
        raise HTTPException(400, "Duplicate submission.")
